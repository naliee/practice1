...
# 적 enemy 캐릭터 로딩
enemy = pygame.image.load("C:\\Users\\이벌브소프트\\Desktop\\PythonWorkspace\\pygame.basic\\enemy.png")
enemy_size = enemy.get_rect().size  # 이미지 크기 구해 옴
...
while running:
    ...
    dt = clock.tick(60) # 게임 화면의 초당 프레임 수를 설정. 작아질수록 뚝뚝 끊기는 느낌
    ...
    # 충돌 처리
    character_rect = character.get_rect()   # 이미지 크기 구하기
    character_rect.left = character_x_pos
    character_rect.top = character_y_pos

    enemy_rect = enemy.get_rect()   # 이미지 (가져옴)크기 구하기
    enemy_rect.left = enemy_x_pos
    enemy_rect.top = enemy_y_pos

    # 충돌 체크
    if character_rect.colliderect(enemy_rect): # 충돌 했느냐
        print("충돌했어요")
        running = False
    ...
    # 경과 시간(ms)을 1000으로 나누어서 초(s) 단위로 표시시

                            # 출력할 글자, True, 글자 색상
    timer = game_font.render(str(int(total_time - elapsed_time)), True, (255, 255, 255))
                            # sec은 자르기 위해 int로 변환, 그 변환값을 다시 string으로
